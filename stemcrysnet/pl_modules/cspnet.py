...
from stemcrysnet.common.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPSTEMNet(nn.Module):
    ...
    def __init__(
        ...
        super(CSPSTEMNet, self).__init__()
        # print('CSPSTEMNet latent dim**************', latent_dim) # stem_dim512 + time_dim256
        # print('CSPSTEMNet hidden dim**************', hidden_dim) # 512
        self.ip = ip
        self.smooth = smooth
        if self.smooth:
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph, stem_feat):
        ...
        if t is None:
            ...
            stem_per_atom = stem_feat[node2graph]
            node_features = torch.cat([node_features, t_per_atom, stem_per_atom], dim=1)
...
class CSPSTEMAddNet(nn.Module):
    ...
    def __init__(
        ...
        super(CSPSTEMAddNet, self).__init__()
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph, stem_feat):
        ...
        # cat stem_feat for each node_features group
        node_features = [torch.cat([node_features[i], stem_feat[i].reshape(1,-1)], dim=0) for i in range(len(node_features))]
